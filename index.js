...
app.get("/", (req, res) => {
  res.send("Welcome to Sam-motors");
});

const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.aiqic.mongodb.net/sam-motors?retryWrites=true&w=majority`;
...
client.connect((err) => {
  const serviceCollection = client.db("sam-motors").collection("services");
  const reviewCollection = client.db("sam-motors").collection("reviews");
  const adminsCollection = client.db("sam-motors").collection("admins");
  const orderCollection = client.db("sam-motors").collection("orders");
